...
<div class="field-guide-wrapper">
  <nav class="field-guide-sidenav {{if @controller.showSideNav "field-guide-sidenav-always-show"}}">
    <LinkTo @route="show" @model="index" class="home-link">
      {{#if @controller.fieldGuideConfig.logo}}
        <img class="field-guide-logo" src={{@controller.fieldGuideConfig.logo}} alt="{{@controller.fieldGuideConfig.name}} Field Guide Logo">
      {{else}}
        <span class="field-guide-fake-logo">{{@controller.fieldGuideConfig.name}}</span>
      {{/if}}
      <span>
        {{#if @controller.fieldGuideConfig.tagLine}}
          {{@controller.fieldGuideConfig.tagLine}}
        {{else}}
          {{@controller.fieldGuideConfig.name}} Field Guide
        {{/if}}
      </span>
    </LinkTo>
  ...
  <div class="field-guide-wrapper-full-height">
    <main>
      {{outlet}}
      <button class="field-guide-show-sidenav" {{action (mut @controller.showSideNav) (not @controller.showSideNav)}}>
